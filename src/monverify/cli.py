...
def fetch_wos(
    ...
    database_id: str | None = typer.Option(None, help="Defaults to rules file (WOK)."),
    omega: Path | None = typer.Option(None, exists=True, help="OMEGA CSV: verify its non-RC WoS IDs instead of discovery."),
    rules_path: Path = typer.Option(Path("config/rules_2025.yaml"), exists=True),
    output: Path = typer.Option(Path("data/raw/wos_2025_compact.json")),
    no_cache: bool = typer.Option(False, help="Ignore raw page cache."),
    verbose: bool = typer.Option(False),
) -> None:
    """Fetch WoS Expanded evidence and save a compact MON Verify JSON file."""
    _logging(verbose)
...
def verify(
    ...
    scival_csv: Path = typer.Option(..., "--scival-csv", exists=True, help="Header-first SciVal CSV with Number of Institutions."),
    wos_json: list[Path] = typer.Option([], "--wos-json", exists=True, help="Repeat for discovery and OMEGA-ID compact/raw WoS JSON files."),
    ministry: Path | None = typer.Option(None, exists=True, help="Optional MON workbook; otherwise use versioned claim in rules_2025.yaml."),
    rules_path: Path = typer.Option(Path("config/rules_2025.yaml"), exists=True),
    output_dir: Path = typer.Option(Path("data/output")),
) -> None:
    outputs = _run_verification(ministry, omega, quartiles, rules_path, output_dir, wos_json, scival_csv)
