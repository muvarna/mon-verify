...
if mode == "Uploaded/cached data":
    ...
    with col4:
        wos_uploads = st.file_uploader("Optional WoS JSON files", type=["json"], accept_multiple_files=True, help="You can upload both institution-discovery and OMEGA-ID verification JSON files.")
    with col5:
        scopus_uploads = st.file_uploader("Optional Scopus JSON files", type=["json"], accept_multiple_files=True, help="You can upload both institution-discovery and OMEGA-ID verification JSON files.")
else:
    st.subheader("Live API retrieval")
    workflows = st.multiselect("Retrieval workflows", ["Independent institutional discovery", "Verify OMEGA identifiers"], default=["Independent institutional discovery", "Verify OMEGA identifiers"])
    wos_key = st.text_input("WoS Expanded API key", value=_secret("WOS_API_KEY"), type="password")
    scopus_key = st.text_input("Scopus API key", value=_secret("SCOPUS_API_KEY"), type="password")
    scopus_insttoken = st.text_input("Optional Scopus institution token", value=_secret("SCOPUS_INSTTOKEN"), type="password")
            ...
            if wos_key:
                ...
                if "Independent institutional discovery" in workflows:
                    with st.spinner("WoS: independent institutional discovery..."):
                        ...
                        wrapper = {"mode": "institution_discovery","query": query,"database_id": WOS_DATABASE_ID,"retrieved_at": datetime.now(timezone.utc).isoformat(),"pages": pages}
                ...
                if "Verify OMEGA identifiers" in workflows:
                    with st.spinner("WoS: verifying OMEGA identifiers..."):
                        ...
                        st.session_state["wos_live_files"].append({"mode": "omega_ids","database_id": WOS_DATABASE_ID,"requested_ids": ids,"retrieved_at": datetime.now(timezone.utc).isoformat(),"pages": pages})
            ...
            if scopus_key:
                ...
                if "Independent institutional discovery" in workflows:
                    with st.spinner("Scopus: independent institutional discovery..."):
                        ...
                        st.session_state["scopus_live_files"].append({"mode": "institution_discovery","query": query,"retrieved_at": datetime.now(timezone.utc).isoformat(),"pages": pages})
                ...
                if "Verify OMEGA identifiers" in workflows:
                    with st.spinner("Scopus: verifying OMEGA identifiers..."):
                        ...
                        st.session_state["scopus_live_files"].append({"mode": "omega_ids","requested_ids": ids,"retrieved_at": datetime.now(timezone.utc).isoformat(),"abstracts": abstracts})
...
if st.button("Run verification", disabled=not (ministry_upload and omega_upload and quartile_upload)):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        pubs = build_canonical_publications(omega_path=omega_path,quartiles_path=quartile_path,rules_path=RULES_PATH,wos_json=wos_paths,scopus_json=scopus_paths)
...
if "pubs" in st.session_state:
    ...
    c1.metric("Publications", pub_count if pub_count is not None else "unresolved", (pub_count - claim.publication_count if pub_count is not None else None))
    q1w = agg["weighted"]["a1"]
    q2w = agg["weighted"]["a2"]
    score = agg["a_score"]
    ...
    st.caption(f"Candidates: {agg['candidate_publication_count']} · confirmed eligible: {agg['confirmed_publication_count']} · excluded: {agg['excluded_publication_count']} · unresolved eligibility: {agg['unresolved_eligibility_count']}")
